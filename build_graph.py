...
def build_q_value(make_obs_ph, q_func, num_actions, scope="deepq", reuse=None):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        observations_ph = U.ensure_tf_input(make_obs_ph("observation"))
        q_values = q_func(observations_ph.get(), num_actions, scope="q_func")
        q_val = U.function(inputs=[observations_ph],
                           outputs=q_values)
        return q_val
...
`       See the top of the file for details.
    ...
    q_val_f = build_q_value(make_obs_ph, q_func, num_actions, scope=scope, reuse=reuse)
    with tf.variable_scope(scope, reuse=reuse):
        ...
        return act_f, train, update_target, {'q_values': q_values}, q_val_f
